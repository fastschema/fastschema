			...
			Expect: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(utils.EscapeQuery("SELECT `groups`.`id`, `groups`.`name` FROM `groups`")).
					WillReturnRows(mock.NewRows([]string{"id", "name"}).
						AddRow(11, "Group 11").
						AddRow(22, "Group 22"))
				mock.ExpectQuery(utils.EscapeQuery("SELECT `t1`.`groups` AS groups_id, `users`.`id`, `users`.`username`, `users`.`email`, `users`.`first_name`, `users`.`last_name`, `users`.`bio`, `users`.`password`, `users`.`active`, `users`.`provider`, `users`.`provider_id`, `users`.`provider_username`, `users`.`provider_profile_image`, `users`.`avatar_id`, `users`.`spouse_id`, `users`.`partner_id`, `users`.`workplace_id`, `users`.`room_id`, `users`.`parent_id`, `users`.`created_at`, `users`.`updated_at`, `users`.`deleted_at`, `users`.`name`, `users`.`status`, `users`.`approved`, `users`.`age`, `users`.`json`, `users`.`deleted` FROM `users` JOIN `groups_users` AS `t1` ON `t1`.`users` = `users`.`id` WHERE `t1`.`groups` IN (?, ?) ORDER BY `users`.`id` ASC")).
					WithArgs(11, 22).
			...
			Expect: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(utils.EscapeQuery("SELECT `users`.`id`, `users`.`name` FROM `users`")).
					WillReturnRows(mock.NewRows([]string{"id", "name"}).
						...
						AddRow(testUserUUID2, "Jane").
						AddRow(testUserUUID3, "Bob"))
				mock.ExpectQuery(utils.EscapeQuery("SELECT `t1`.`followers` AS followers_id, `users`.`id`, `users`.`username`, `users`.`email`, `users`.`first_name`, `users`.`last_name`, `users`.`bio`, `users`.`password`, `users`.`active`, `users`.`provider`, `users`.`provider_id`, `users`.`provider_username`, `users`.`provider_profile_image`, `users`.`avatar_id`, `users`.`spouse_id`, `users`.`partner_id`, `users`.`workplace_id`, `users`.`room_id`, `users`.`parent_id`, `users`.`created_at`, `users`.`updated_at`, `users`.`deleted_at`, `users`.`name`, `users`.`status`, `users`.`approved`, `users`.`age`, `users`.`json`, `users`.`deleted` FROM `users` JOIN `followers_following` AS `t1` ON `t1`.`following` = `users`.`id` WHERE `t1`.`followers` IN (?, ?, ?) ORDER BY `users`.`id` ASC")).
					WithArgs(testUserUUID1, testUserUUID2, testUserUUID3).
			...
			Expect: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(utils.EscapeQuery("SELECT `users`.`id`, `users`.`name` FROM `users`")).
					WillReturnRows(mock.NewRows([]string{"id", "name"}).
						...
						AddRow(testUserUUID2, "Jane").
						AddRow(testUserUUID3, "Bob"))
				mock.ExpectQuery(utils.EscapeQuery("SELECT `t1`.`following` AS following_id, `users`.`id`, `users`.`username`, `users`.`email`, `users`.`first_name`, `users`.`last_name`, `users`.`bio`, `users`.`password`, `users`.`active`, `users`.`provider`, `users`.`provider_id`, `users`.`provider_username`, `users`.`provider_profile_image`, `users`.`avatar_id`, `users`.`spouse_id`, `users`.`partner_id`, `users`.`workplace_id`, `users`.`room_id`, `users`.`parent_id`, `users`.`created_at`, `users`.`updated_at`, `users`.`deleted_at`, `users`.`name`, `users`.`status`, `users`.`approved`, `users`.`age`, `users`.`json`, `users`.`deleted` FROM `users` JOIN `followers_following` AS `t1` ON `t1`.`followers` = `users`.`id` WHERE `t1`.`following` IN (?, ?, ?) ORDER BY `users`.`id` ASC")).
					WithArgs(testUserUUID1, testUserUUID2, testUserUUID3).
			...
			Expect: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(utils.EscapeQuery("SELECT `users`.`id`, `users`.`name` FROM `users`")).
					WillReturnRows(mock.NewRows([]string{"id", "name"}).
						...
						AddRow(testUserUUID2, "Jane").
						AddRow(testUserUUID3, "Bob"))
				mock.ExpectQuery(utils.EscapeQuery("SELECT `t1`.`friends` AS friends_id, `users`.`id`, `users`.`username`, `users`.`email`, `users`.`first_name`, `users`.`last_name`, `users`.`bio`, `users`.`password`, `users`.`active`, `users`.`provider`, `users`.`provider_id`, `users`.`provider_username`, `users`.`provider_profile_image`, `users`.`avatar_id`, `users`.`spouse_id`, `users`.`partner_id`, `users`.`workplace_id`, `users`.`room_id`, `users`.`parent_id`, `users`.`created_at`, `users`.`updated_at`, `users`.`deleted_at`, `users`.`name`, `users`.`status`, `users`.`approved`, `users`.`age`, `users`.`json`, `users`.`deleted` FROM `users` JOIN `friends_user` AS `t1` ON `t1`.`user` = `users`.`id` WHERE `t1`.`user` IN (?, ?, ?) ORDER BY `users`.`id` ASC")).
					WithArgs(testUserUUID1, testUserUUID2, testUserUUID3).
