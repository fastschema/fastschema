	...
	tests := []testutils.DBTestDeleteData{
		{
			...
			Expect: func(t *testing.T, m db.Model) {
				entity, err := m.Query().Where(db.EQ("id", 1)).Only()
				assert.Error(t, err)
				assert.Nil(t, entity)
			},
		},
		...
		},
		{
			...
			WantErr: false,
			Expect: func(t *testing.T, m db.Model) {
				card := utils.Must(utils.Must(client.Model("card")).Query().Where(db.EQ("id", 1)).Only())
			...
			WantErr: false,
			Expect: func(t *testing.T, m db.Model) {
				users := utils.Must(utils.Must(client.Model("user")).Query().Where(db.EQ("id", 1)).Get())
				assert.Equal(t, 0, len(users))
				groupsUsers := utils.Must(utils.Must(client.Model("groups_users")).Query().Where(db.EQ("users", 1)).Get())
