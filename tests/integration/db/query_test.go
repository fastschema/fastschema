	...
	tests := []testutils.DBTestQueryData{
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				return []*schema.Entity{
					utils.Must(m.Query(db.EQ("id", user1ID)).First()),
					utils.Must(m.Query(db.EQ("id", user2ID)).First()),
				}
			},
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				assert.NoError(t, err2)
				return []*schema.Entity{utils.Must(m.Query(db.EQ("id", user1ID)).First())}
			},
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				return []*schema.Entity{
					utils.Must(m.Query(db.EQ("id", user22ID)).First()),
					utils.Must(m.Query(db.EQ("id", user2ID)).First()),
					utils.Must(m.Query(db.EQ("id", user1ID)).First()),
				}
			},
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				car2 := utils.Must(m.Query(db.EQ("id", car2ID)).First())
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				return []*schema.Entity{schema.NewEntity(user1ID).Set("name", "User 1").Set("pets", []*schema.Entity{
					utils.Must(utils.Must(client.Model("pet")).Query(db.EQ("id", pet1ID)).First()),
					utils.Must(utils.Must(client.Model("pet")).Query(db.EQ("id", pet2ID)).First()),
				})}
			},
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				return []*schema.Entity{
					schema.NewEntity(node1ID).Set("name", "Node 1").Set("children", []*schema.Entity{
						utils.Must(m.Query(db.EQ("id", node2ID)).First()),
						utils.Must(m.Query(db.EQ("id", node3ID)).First()),
					}),
				}
			},
		},
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				card1 := utils.Must(utils.Must(client.Model("card")).Query(db.EQ("id", card1ID)).First())
				return []*schema.Entity{
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1 := utils.Must(utils.Must(client.Model("user")).Query(db.EQ("id", user1ID)).First())
				user2 := utils.Must(utils.Must(client.Model("user")).Query(db.EQ("id", user2ID)).First())
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				a1 := utils.Must(utils.Must(m.Mutation()).Where(db.EQ("id", node3ID)).Update(schema.NewEntity().Set("prev_id", 1)))
				assert.Equal(t, 1, a1)

				a2 := utils.Must(utils.Must(m.Mutation()).Where(db.EQ("id", node4ID)).Update(schema.NewEntity().Set("prev_id", 2)))
				assert.Equal(t, 1, a2)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				a1 := utils.Must(utils.Must(m.Mutation()).Where(db.EQ("id", user1ID)).Update(schema.NewEntity().Set("spouse_id", user2ID)))
				assert.Equal(t, 1, a1)

				a2 := utils.Must(utils.Must(m.Mutation()).Where(db.EQ("id", user2ID)).Update(schema.NewEntity().Set("spouse_id", user1ID)))
				assert.Equal(t, 1, a2)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1 := utils.Must(userModel.Query(db.EQ("id", user1ID)).First())
				user2 := utils.Must(userModel.Query(db.EQ("id", user2ID)).First())
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				group1 := utils.Must(groupModel.Query(db.EQ("id", group1ID)).First())
				group2 := utils.Must(groupModel.Query(db.EQ("id", group2ID)).First())
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				_, err := utils.Must(m.Mutation()).Where(db.EQ("id", user1ID)).Update(schema.NewEntity(user1ID).Set("following", []*schema.Entity{
					schema.NewEntity(user2ID),
					schema.NewEntity(user3ID),
				}))
				assert.NoError(t, err)

				_, err = utils.Must(m.Mutation()).Where(db.EQ("id", user2ID)).Update(schema.NewEntity(user2ID).Set("following", []*schema.Entity{
					schema.NewEntity(user3ID),
					schema.NewEntity(user4ID),
				}))
				assert.NoError(t, err)

				_, err = utils.Must(m.Mutation()).Where(db.EQ("id", user3ID)).Update(schema.NewEntity(user3ID).Set("following", []*schema.Entity{
					schema.NewEntity(user4ID),
				}))
				assert.NoError(t, err)

				user2 := utils.Must(m.Query(db.EQ("id", user2ID)).First())
				user3 := utils.Must(m.Query(db.EQ("id", user3ID)).First())
				user4 := utils.Must(m.Query(db.EQ("id", user4ID)).First())
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1 := utils.Must(m.Query(db.EQ("id", user1ID)).First())
				user2 := utils.Must(m.Query(db.EQ("id", user2ID)).First())
				user3 := utils.Must(m.Query(db.EQ("id", user3ID)).First())
				user4 := utils.Must(m.Query(db.EQ("id", user4ID)).First())
				user5 := utils.Must(m.Query(db.EQ("id", user5ID)).First())
