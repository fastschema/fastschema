	...
	tests := []DBTestQueryData{
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID, err1 := m.Mutation().Create(Ctx(), schema.NewEntity().Set("name", "John Doe").Set("username", "john"))
				assert.NoError(t, err1)
				user2ID, err2 := m.Mutation().Create(Ctx(), schema.NewEntity().Set("name", "Jane Doe").Set("username", "jane"))
		...
		},
		{
			...
			ClearTables: []string{"users"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID, err1 := m.Mutation().Create(Ctx(), schema.NewEntity().Set("name", "John Doe").Set("age", 10).Set("username", "john"))
				assert.NoError(t, err1)
				_, err2 := m.Mutation().Create(Ctx(), schema.NewEntity().Set("name", "Jane Doe").Set("age", 2).Set("username", "jane"))
				assert.NoError(t, err2)
				return []*schema.Entity{utils.Must(m.Query(db.EQ("id", user1ID)).First(Ctx()))}
			},
		},
		{
			...
			ClearTables: []string{"users"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1"}`))
				assert.True(t, user1ID > 0)
				user22ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 2 2", "username": "user22"}`))
				assert.True(t, user22ID > 0)
				user2ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2"}`))
				assert.True(t, user2ID > 0)
				user3ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 3", "username": "user3"}`))
				assert.True(t, user3ID > 0)
				user4ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 4", "username": "user4"}`))
		...
		},
		{
			...
			ClearTables: []string{"users"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 1", "age": 10, "username": "user1"}`))
				assert.True(t, user1ID > 0)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 1", "groups": [{"id": 1}], "username": "user1"}`))
				assert.True(t, user1ID > 0)
				user2ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 2", "groups": [{"id": 2}], "username": "user2"}`))
				assert.True(t, user2ID > 0)
		...
		},
		{
			...
			ClearTables: []string{"users", "pets"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1"}`))
				assert.True(t, user1ID > 0)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1"}`))
				user2ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2"}`))
				assert.True(t, user1ID > 0)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				a1 := utils.Must(m.Mutation().Where(db.EQ("id", node3ID)).Update(Ctx(), schema.NewEntity().Set("prev_id", 1)))
				assert.Equal(t, 1, a1)

				a2 := utils.Must(m.Mutation().Where(db.EQ("id", node4ID)).Update(Ctx(), schema.NewEntity().Set("prev_id", 2)))
				assert.Equal(t, 1, a2)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				a1 := utils.Must(m.Mutation().Where(db.EQ("id", user1ID)).Update(Ctx(), schema.NewEntity().Set("spouse_id", user2ID)))
				assert.Equal(t, 1, a1)

				a2 := utils.Must(m.Mutation().Where(db.EQ("id", user2ID)).Update(Ctx(), schema.NewEntity().Set("spouse_id", user1ID)))
				assert.Equal(t, 1, a2)

				return []*schema.Entity{
					schema.NewEntity(user1ID).Set("name", "User 1").Set("spouse_id", uint64(2)).Set("spouse", utils.Must(m.Query(db.EQ("id", user2ID)).First(Ctx()))),
					schema.NewEntity(user2ID).Set("name", "User 2").Set("spouse_id", uint64(1)).Set("spouse", utils.Must(m.Query(db.EQ("id", user1ID)).First(Ctx()))),
				}
			},
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1ID := utils.Must(userModel.CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1", "groups": [{"id": 1}]}`))
				assert.True(t, user1ID > 0)
				user2ID := utils.Must(userModel.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "groups": [{"id": 1}, {"id": 2}]}`))
				assert.True(t, user2ID > 0)

				user3ID := utils.Must(userModel.CreateFromJSON(Ctx(), `{"name": "User 3", "username": "user3"}`))
				assert.True(t, user3ID > 0)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user5ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 5", "username": "user5"}`))
				assert.True(t, user5ID > 0)

				_, err := m.Mutation().Where(db.EQ("id", user1ID)).Update(Ctx(), schema.NewEntity(user1ID).Set("following", []*schema.Entity{
					schema.NewEntity(user2ID),
					schema.NewEntity(user3ID),
				}))
				assert.NoError(t, err)

				_, err = m.Mutation().Where(db.EQ("id", user2ID)).Update(Ctx(), schema.NewEntity(user2ID).Set("following", []*schema.Entity{
					schema.NewEntity(user3ID),
					schema.NewEntity(user4ID),
				}))
				assert.NoError(t, err)

				_, err = m.Mutation().Where(db.EQ("id", user3ID)).Update(Ctx(), schema.NewEntity(user3ID).Set("following", []*schema.Entity{
					schema.NewEntity(user4ID),
				}))
				assert.NoError(t, err)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user2ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "following": [{"id": 1}]}`))
				assert.True(t, user2ID > 0)

				user3ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 3", "username": "user3", "following": [{"id": 1}, {"id": 2}]}`))
				assert.True(t, user3ID > 0)

				user4ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 4", "username": "user4", "following": [{"id": 1}, {"id": 2}, {"id": 3}]}`))
				assert.True(t, user4ID > 0)
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user2ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "friends": [{"id": 1}]}`))
				assert.True(t, user2ID > 0)

				user3ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 3", "username": "user3", "friends": [{"id": 1}, {"id": 2}]}`))
				assert.True(t, user3ID > 0)

				user4ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 4", "username": "user4", "friends": [{"id": 1}]}`))
				assert.True(t, user4ID > 0)

				user5ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 5", "username": "user5", "friends": [{"id": 4}]}`))
				assert.True(t, user5ID > 0)

				user6ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 6", "username": "user6"}`))
		...
		},
		{
			...
			ClearTables: []string{"users", "cards"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1", "age": 5}`))
				user2ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "age": 8}`))
				assert.True(t, user1ID > 0)
		...
		},
		{
			...
			ClearTables: []string{"users", "pets"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				user1ID := utils.Must(utils.Must(client.Model("user")).CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1", "age": 8}`))
				assert.True(t, user1ID > 0)
				pet1ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "Pet 1", "owner": {"id": 1}}`))
				assert.True(t, pet1ID > 0)
				pet2ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "Pet 2", "owner": {"id": 1}}`))
		...
		},
		{
			...
			Prepare: func(t *testing.T, client db.Client, m db.Model) []*schema.Entity {
				...
				user1ID := utils.Must(userModel.CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1", "age": 8, "groups": [{"id": 1}]}`))
				assert.True(t, user1ID > 0)
				user2ID := utils.Must(userModel.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "age": 5, "groups": [{"id": 1}, {"id": 2}]}`))
				assert.True(t, user2ID > 0)
...
func DBCountNode(t *testing.T, client db.Client) {
	tests := []DBTestCountData{
		...
		},
		{
			...
			ClearTables: []string{"users"},
			Prepare: func(t *testing.T, client db.Client, m db.Model) int {
				user1ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 1", "username": "user1", "status": "offline"}`))
				assert.True(t, user1ID > 0)

				user2ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 2", "username": "user2", "status": "online"}`))
				assert.True(t, user2ID > 0)

				user3ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 3", "username": "user3", "status": "offline"}`))
				assert.True(t, user3ID > 0)

				user4ID := utils.Must(m.CreateFromJSON(Ctx(), `{"name": "User 4", "username": "user4", "status": "online"}`))
				assert.True(t, user4ID > 0)
