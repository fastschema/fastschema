	...
	tests := []testutils.DBTestCreateData{
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				entity, err := m.Query(db.EQ("id", e.ID())).Only()
	...
	tests := []testutils.DBTestCreateData{
		{
			...
			Prepare: func(t *testing.T) {
				...
				car1 := utils.Must(utils.Must(client.Model("car")).Query(db.EQ("id", car1ID)).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				entity := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				...
				user3 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				node2 := utils.Must(m.Query(db.EQ("id", 2)).Only())
				node3 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				pet1 := utils.Must(utils.Must(client.Model("pet")).Query(db.EQ("id", 1)).Only())
				user2 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				pet1 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
				user2 := utils.Must(utils.Must(client.Model("user")).Query(db.EQ("id", 2)).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				node1 := utils.Must(m.Query(db.EQ("id", 1)).Only())
				node2 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				node4 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
				node2 := utils.Must(m.Query(db.EQ("id", 2)).Only())
				node3 := utils.Must(m.Query(db.EQ("id", 3)).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				node1 := utils.Must(m.Query(db.EQ("id", 1)).Only())
				node3 := utils.Must(m.Query(db.EQ("id", 3)).Only())
				node4 := utils.Must(m.Query(db.EQ("id", 4)).Only())
				node5 := utils.Must(m.Query(db.EQ("id", e.ID())).Only())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, groupEntity *schema.Entity) {
				group1 := utils.Must(m.Query(db.EQ("id", groupEntity.ID())).Only())
				groupsUsers := utils.Must(utils.Must(client.Model("groups_users")).Query(db.EQ("groups", group1.ID())).Get())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, userEntity *schema.Entity) {
				user1 := utils.Must(m.Query(db.EQ("id", userEntity.ID())).Only())
				groupsUsers := utils.Must(utils.Must(client.Model("groups_users")).Query(db.EQ("users", user1.ID())).Get())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, userEntity *schema.Entity) {
				user3 := utils.Must(m.Query(db.EQ("id", userEntity.ID())).Only())

				friendsUsers := utils.Must(utils.Must(client.Model("friends_user")).Query(db.EQ("user", user3.ID())).Get())
		...
		},
		{
			...
			Expect: func(t *testing.T, m db.Model, e *schema.Entity) {
				...
				groupsUsers := utils.Must(utils.Must(client.Model("groups_users")).Query(db.EQ("users", user3.ID())).Get())
