...
func main() {
	...
	tag1 := utils.Must(db.Builder[Tag](app.DB()).Create(ctx, fs.Map{
		"name": "Technology",
	...
	// Update blog
	updatedBlog1 := utils.Must(db.Builder[Blog](app.DB()).
		Where(db.EQ("id", blog1.ID)).
		Update(ctx, fs.Map{
			"vote": 10,
		}))
	...
	// Delete blog
	affected := utils.Must(db.Builder[Blog](app.DB()).
		Where(db.EQ("id", blog1.ID)).
		Delete(ctx))
	fmt.Printf("> Delete blog, affected: %d\n\n", affected)

	// Verify delete
	_, err := db.Builder[Blog](app.DB()).
		Where(db.EQ("id", blog1.ID)).
		First(ctx)
